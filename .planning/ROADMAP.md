...
- [x] **Phase 69: Auto Episode Summary & Lazy Regeneration** — the AI generates and stores `episode_summary` when an episode is completed, and a stale summary is regenerated before it is used as prior-episode context for a later episode (lazy regen via the `episode_summary_stale` flag)
